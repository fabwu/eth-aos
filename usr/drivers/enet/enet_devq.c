...
errval_t enet_rx_queue_create(struct enet_queue **q, enet_t *dev)
{
    ...
    err = paging_map_frame_attr(
        get_current_paging_state(), (void **)&(rxq->desc_mem.vbase), tot_size,
        rxq->desc_mem.mem, VREGION_FLAGS_READ_WRITE_NOCACHE, NULL, NULL);
    if (err_is_fail(err)) {
...
errval_t enet_tx_queue_create(struct enet_queue **q, struct enet_t *dev)
{
    ...
    err = paging_map_frame_attr(
        get_current_paging_state(), (void **)&(txq->desc_mem.vbase), tot_size,
        txq->desc_mem.mem, VREGION_FLAGS_READ_WRITE_NOCACHE, NULL, NULL);
    if (err_is_fail(err)) {
